...
public class DynamicAdapter extends RecyclerView.Adapter<DynamicAdapter.TextHolder> {
    ...
    public TextHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        return new TextHolder(mContent.getLayoutInflater().inflate(R.layout.item_test_list_dt, parent, false));
    }

    @Override
    public void onBindViewHolder(final TextHolder holder,final int position) {
        holder.hend.setImageResource(R.mipmap.ic_action_bill);//设置头部的文字
        holder.name.setText(UtilDt.getName(position));//设置名称
        holder.imageView.setImageResource(R.mipmap.ic_action_bill);
...
//                    如果内容显示的行数大于限定显示行数
            ...
            holder.content.setText(UtilDt.getContent(position));//用Util中的getContent方法获取内容
        }else{
//            如果之前已经初始化过了，则使用保存的状态，无需在获取一次
            ...
            holder.content.setText(UtilDt.getContent(position));
        }
...
//        设置显示和收起的点击事件
    ...
    public class TextHolder extends RecyclerView.ViewHolder {
        ...
        public TextHolder(View itemView) {
            super(itemView);
//            绑定xml布局中的控件
            hend = (ImageView) itemView.findViewById(R.id.iv_hend);
